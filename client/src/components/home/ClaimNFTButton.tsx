...
import { GET_TOTAL_PAGE_KEY, GET_USER_NFTS_KEY } from "@/hooks/useUserNfts";
...
function ClaimNFTButton({}: Props) {
  ...
  const handleClaimNFT = async () => {
    ...
    try {
      ...
      await Promise.all([
        (
          await connectNFTContract?.giveAway(await signerSigner.getAddress())
        ).wait(),
      ]);
  ...
  return (
    <Button className="px-3 py-1" onClick={handleClaimNFT}>
      {!isLoading ? (
